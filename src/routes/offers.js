...
router.post('/', authMiddleware, [
  ...
  try {
    ...
    sendNotification('newOffer', notify.newOffer({
      ...
      buyerName:     buyerRows[0]?.name || 'Bir kullanıcı',
...
router.patch('/:id/respond', authMiddleware, [
  ...
  try {
    ...
    const { rows: offerRows } = await client.query(`
      SELECT o.*, l.seller_id, l.title AS crop_name, l.unit, l.status AS listing_status,
             buyer.name AS buyer_name, seller.name AS seller_name
      FROM offers o
      JOIN listings l ON l.id=o.listing_id
      JOIN users buyer ON buyer.id=o.buyer_id
...
router.patch('/:id/cancel-counter', authMiddleware, async (req, res, next) => {
  ...
  try {
    await client.query('BEGIN');
    const { rows: offerRows } = await client.query(`
      SELECT o.*, l.seller_id, l.title AS crop_name,
             buyer.name AS buyer_name, seller.name AS seller_name
      FROM offers o
      JOIN listings l ON l.id=o.listing_id
      JOIN users buyer ON buyer.id=o.buyer_id
